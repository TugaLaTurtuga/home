...
function saveGameData() {
  localStorage.setItem("_GamblingSimulator", JSON.stringify(player));
  displayGameData();
}

// Load or reset player data
function loadGameData(loadData = true) {
  let savedData = localStorage.getItem("_GamblingSimulator");

  if (
    !loadData &&
    confirm(
      "Are you sure you want to reset the game? This will delete all your progress.",
    )
  ) {
    localStorage.removeItem("_GamblingSimulator");
...
function updatePlayerData(gameData) {
  ...
  if (player.remainingLoanTime !== 0 && player.remainingLoanValue !== 0) {
    takeLoan(
      player.remainingLoanValue / (1 + player.loanInterest / 100),
      player.loanInterest,
  ...
  reader.onload = (event) => {
    ...
    JSZip.loadAsync(zipData)
      ...
      .then((base85Encoded) => {
        ...
        const invalidChars = playerDataString.match(
          /[^"\{\}\[\],:0-9a-zA-Z.\-+_ ]/g,
        );
        if (invalidChars) {
          console.error(
            "Invalid characters detected in player data:",
            invalidChars,
          );
          return;
        }
