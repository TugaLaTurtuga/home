function formatCost(cost) {
  ...
  switch (true) {
    case cost >= 1e39:
      const power = Math.floor(Math.log10(cost));
  ...
  return (
    "$" +
    ((cost / magnitude) % 1 === 0
      ? (cost / magnitude).toFixed(0) + suffix
      : (cost / magnitude).toFixed(2) + suffix)
  );
}

// Update the displayed balance in both sections
function updateBalance() {
  document.getElementById("money-count").innerText = formatCost(
