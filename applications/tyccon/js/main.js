...
window.addEventListener('keyup', (e) => {
    if (e.key === carryingLetter || e.key === carryingDeliveryLetter) {
        ...
        if (carrying) {
            for (const [key, other] of Object.entries(player.items.utensils)) {
                if (key === 'trash' && pos[0] === other.pos[0] && pos[1] === other.pos[1]) {
                    // If the user is in the trash bin, discard the item
                    console.log('Discarding:', carrying.name);
                    carrying = null;
                    render();
                    return;
                }
            ...
            function placeDown(utensil) {
                ...
                console.log('Placing down:', carrying.name, ' at position:', utensil.pos);
                ...
                for (const [x, y] of newTiles) {
                    ...
                    for (const [key, other] of Object.entries(player.items.utensils)) {
                        if (typeof other !== "object") continue;

                        const otherTiles = getCoveredTiles(other.pos, other.size);
                ...
                player.items.utensils[carrying.name] = utensil;
                carrying = null;
                render();
            ...
            if (carrying.data?.size) {
                carrying.data = placeDown(carrying.data); // it's a utensil, so place it down. 
            } else {
                // not a utensil
            }
        }
        ...
        for (const [categoryKey, categoryItems] of Object.entries(player.items)) {
            for (const [key, item] of Object.entries(categoryItems)) {
                ...
                pos[1] >= uY && pos[1] <= uY + uSizeY - 1 ) {
                    ...
                    carrying = {
                        name: item.food[item.holdPos[0]][item.holdPos[1]][0]['name'],
                        data: item.food[item.holdPos[0]][item.holdPos[1]][0]['data']
                    };
